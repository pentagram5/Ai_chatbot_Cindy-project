...
class NER:
    ...
    def create_model(self):
        model = TFBertModel.from_pretrained("monologg/kobert", from_pt=True, num_labels=len(self.label_dict), output_attentions = False, output_hidden_states = False)
        ...
        self.nr_model.compile(optimizer=tf.keras.optimizers.Adam(lr=0.00002), loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=False),
    ...
    def ner_inference(self,test_sentence):  
        self.label_list = []
        self.token_list = []
        self.tokenized_sentence = np.array([self.tokenizer.encode(test_sentence, max_length=self.SEQ_LEN, pad_to_max_length=True, truncation=True)])
    ...
    #global news_date_tags,news_categori_tags,news_answer_date,news_news_tags
        ...
        if len(self.news_date_tags) > 0:
            ...
            lis_color = {'정치':'#FB4538','경제':'#3F18FE','사회':'#AF810E','생활/문화':'#207DFD','세계':'#1FDA3E','It/과학':'#7B84ED'}
                ...
                for i,val in enumerate(cate):
                    for c in self.news_categori_tags:
                        if (c.find(val) >= 0) | (c[0:2] == val[0:2]):
                            #self.rtn_str += '[{}]{}({})'.format(val,news[i],link[i])
                            self.rtn_str += "<tr><td><font color='"+lis_color[val]+"'>[<b>{}</b>]</font></td><td>{}</td><td><a href='{}'><img width='15' height='15' src='./ner/img/link_icon2.png'></a></td></tr>".format(val,news[i],link[i])
    ...
    def get_weather(self,question):
        ...
        if (len(self.loc_tags) == 0) & (len(self.date_tags) == 0):
            tagstate = 'N'
            #print('언제 어디 날씨를 알고 싶으세요?',labels)
            ...
            try:
                ...
                #ttm = soup.find('span',id='wob_ttm').text + '°C'
                ...
                #answer += self.get_weather_form(loc,dts,ttm,pp,hm,ws,dc)

                #answer += '{}({})의 날씨는 \n\t온도 : {}\n\t강수확률:{}\n\t습도:{}\n\t풍속:{} \n\t기상 : {}'.format(loc,dts,ttm,pp,hm,ws,dc)
                if int(pp.replace('%','')) > 30:
                    #print('1')
                    answer += self.get_weather_form(loc, dts, ttm, pp, hm, ws, dc, 2)
                    #answer += '\n {}은 우산 챙기시는게 좋겠네요.'.format(' '.join(self.answer_date).replace('?',''))
                else:
    ...
    def get_food(self,question):
        ...
        if (len(j_food_loc) > 0 ) | (len(j_food_nm) > 0 ):
            ...
            #return np.array(df_results.iloc[0:3, 1:].values.tolist())
    ...
    def get_weather_form(self,loc,dts,ttm,pp,hm,ws,dc,w_type):
        #rtn_html = ''
        #self.rtn_html = "<div style='border:1px solid black;'>" +  "<div style='padding-left:7px;'>" + "<p><b><font size='4'>"+loc+"</font></b></p>"+"<p style='line-height:1px'><b><font size='2' color='gray'>("+dts+")</font></b></p>"+"</div>"+"<div>"+"<div stlye='border:1px solid;'>"+"<img style='float:left;height:64px;width:64px' alt='"+dc+"' src='C:/Users/admin/PycharmProjects/new_seq/ner/img/"+dc+".png' data-atf='1'>"+ dc+"</div>"+ "<span><b><font size='7'>"+ttm+"</font> <font color='red'>°C</font></b></span>"+ "</div>"+"<div style='width:130px;padding-left:58px;line-height:17px;padding-top:1px;min-width:43%'>"+"<ul style='list-style:none;'>"+ "<li><font size='2' color='gray'>강수확률 : "+pp+"%</font></li>"+ "<li><font size='2' color='gray'>습도 : "+hm+"%</font></li>"+ "<li><font size='2' color='gray'>풍속 : "+ws+"</font></li>"+ "</ul>"+ "</div>"+"</div>"
        #self.rtn_html = "<table width='150px' style='background-color:pink;border:1px solid;border-radius:25px 25px;padding:5px 5px 5px 5px;'><tr><td colspan='2'><b><font size='4'>"+loc+"</font></b></td></tr><tr><td colspan='2'><b><font size='3' color='gray'>"+dts+"</font></b></td></tr><tr><td rowspan='2'><img style='float:left;height:64px;width:64px' src='C:/Users/admin/PycharmProjects/new_seq/ner/img/"+dc+".png' data-atf='1'></td><td><font size='3'>"+dc+"</font></td></tr><tr><td><b><font size='5'>"+ttm+"</font> <font color='red'>°C</font></b></td></tr><tr><td colspan='2'><ul style='list-style:none;'><li><font size='3' color='gray'>강수확률 : "+pp+"</font></li><li><font size='3' color='gray'>습도 : "+hm+"</font></li><li><font size='3' color='gray'>풍속 : "+ws+"</font></li></ul></td></tr></table>"
        print('w_type=',w_type)
        if w_type == 1:
            #print('1')
            self.rtn_html = "<table width='80%' height='80%' style='margin-bottom:2px;border-color:#9ABAD9;background-color:snow;padding:5px 5px 5px 5px;'><tr><td colspan='2'><b><font size='4'>"+loc+"</font></b></td></tr><tr><td colspan='2'><b><font size='3' color='gray'>"+dts+"</font></b></td></tr><tr><td rowspan='2'><img style='float:left;height:64px;width:64px' src='C:/Users/admin/PycharmProjects/new_seq/ner/img/"+dc+".png' data-atf='1'></td><td><font size='3'>"+dc+"</font></td></tr><tr><td><b><font size='5'>"+ttm+"</font> <font color='red'>°C</font></b></td></tr><tr><td colspan='2'><ul style='list-style:none;'><li><font size='3' color='gray'>강수확률 : "+pp+"</font></li><li><font size='3' color='gray'>습도 : "+hm+"</font></li><li><font size='3' color='gray'>풍속 : "+ws+"</font></li></ul></td></tr></table>"
        elif w_type == 2:
            #print('2')
...
# ner = NER()
# m = ner.create_model()
# txt = ner.get_food('구로구 소고기 맛집 알려줘~')
# print(txt)
